...
- **Vùng bấm chuẩn 44px cho máy POS 10"** (24/08/2026): luật "≥44×44" trong `CONVENTIONS.md` trước đây chỉ sống ở nơi ai đó *nhớ* ghi đè `h-11` — bản thân `Button`/`Input`/`SelectTrigger`/`TabsList` đều 36px. Nay sửa **từ gốc** ở `packages/ui`: `Button` 40/36/44/40 + tiện ích mới **`tap-44`** (`globals.css`) nới vùng bấm lên 44 bằng lớp giả `::after` trong suốt nên **bố cục và mật độ y nguyên**; `Input` 44px và bỏ `md:text-sm` (chữ 16px mọi khổ màn — bản cũ làm máy POS màn to có chữ *nhỏ hơn* điện thoại). Đường bán hàng: nút −/+ và xoá dòng trong giỏ lên 44 thật (`cart-sidebar`), phóng chữ 10-13px lên 12-16px, nút thu gọn nhóm tùy chọn có sàn chiều cao. **Thẻ bàn**: thẻ chỉ rộng ~166px nên 5 nút icon 28×28 (QR/Lịch sử/Gán/Gộp/Xoá) đã **gom vào một nút ⋮** — `ActionsMenu` chuyển từ `menu/_components/` ra `components/actions-menu.tsx` dùng chung; chỗ trống giải phóng cho "Hủy bàn"/"Thanh toán"/"Mở bàn" lên 44px chữ 14px. Lưới vẫn 6 cột, màn Bán hàng vẫn 16 món.
- **Sổ ghi nhận in phiếu + vá 3 chỗ nuốt lỗi im lặng** (03/09/2026, `edb68d5`+`d1339e4`): sáng 03/09 mất phiếu đặt món **cả hai kiểu** (mất hẳn đơn; đơn nhiều ly ra thiếu tờ) mà **không truy được đơn nào** — vì hệ thống trước đó KHÔNG lưu bất cứ dấu vết nào về việc in. Số liệu máy chủ hoàn toàn sạch (100 đơn, 100×201, bộ đếm ca khớp, 0 lỗ hổng số phiếu), tức **không phải lỗi thao tác**. Ba gốc: ① `MainActivity.printBase64()` khai `void` rồi **vứt kết quả `writeToPrinter()`** → web luôn `return true` dù mất máy in/mất quyền USB; cộng `print_mode=per_item` bắn 2-3 lệnh in liên tiếp không nghỉ. ② `use-websocket.ts` **không hề gửi ping** (máy chủ đã có `pong`) → WiFi rớt kiểu không bắn `onclose` là trạm quầy "chết mà không biết", lệnh in mất vĩnh viễn. ③ Không có nút in lại phiếu đặt món nào. **Vá:** bảng `order_prints` (migration 0016) + `POST /orders/:id/print-ack` (ghi bằng `onConflictDoUpdate`, lần in tốt hơn nâng được lên `ok`, hỏng lại không đè xuống) · `GET /orders/unprinted` + vòng đòi phiếu 45 giây ở trạm quầy (chống trùng bằng **sổ ở máy chủ**, không phải tập RAM) · ping 20 giây + tự nối lại · cầu in USB trả **boolean** thật (APK cũ trả `undefined` → coi là `true` để không hỏng máy chưa cài bản mới) + nghỉ 300ms giữa hai tờ + thử lại 1 lần · `per_item` chỉ in lại **tờ hỏng**, và ném lỗi khi cầu in từ chối → trạm quầy hiện **khối đỏ phải bấm mới tắt** · huy hiệu **CHƯA IN/IN THIẾU** trên thẻ bàn · nút **In lại phiếu đặt món** (`POST /orders/session/:id/reprint` cho bàn, `POST /orders/:id/reprint` cho đơn mang về) với trường **`reprintToken`** riêng · thêm `branchId` vào `order:new` + lọc ở trạm quầy. Khối dựng payload phiếu gom về `services/ticket.service.ts` (trước bị chép đôi). ⚠️ **Cửa 20 phút** ở cả `/unprinted` lẫn huy hiệu: sổ trống KHÔNG có nghĩa là chưa in — thiếu chặn trên này là máy quầy in xối xả cả trăm phiếu cũ lúc vừa tải lại. 1143 đơn cũ đã lấp sổ với `device_label='backfill-truoc-khi-co-so'`.
- **Đơn đã xong thì không còn phiếu đặt món + dấu "IN LẠI" + tách bàn cấp số theo ca** (20/09/2026, `cad0df1`): chiều 20/09 quầy nhả 2 phiếu (trà chanh + trà tắc) và loa đọc to trong khi **không ai gọi món**, mã phiếu lại là dạng cũ `260919-ZFTF`. **Máy không tự in — có người bấm nhầm.** Sổ `order_prints` + log dựng lại từng giây: 17:23:20 đăng nhập → 17:23:40/42/45 lướt trang Đơn hàng 5→6→7 (mỗi trang 2 giây) → **17:23:46.8 `POST /orders/:id/reprint`** → 17:23:47.5 in 2/2 + loa đọc. Đơn bị in tạo **19/09 20:22**, đã `completed`. **Gốc là lỗi thiết kế đợt 03/09**: nút in lại *phiếu đặt món* đặt **sát** nút in *hóa đơn*, cùng cỡ cùng kiểu, chỉ khác icon, **một chạm là ra giấy**, và phiếu in ra **không hề nói nó là bản in lại**. **Vá:** ① đơn `completed`/`cancelled` → máy chủ trả **400 `ORDER_CLOSED`** ở cả `/:id/reprint` lẫn `/session/:id/reprint`, giao diện ẩn nút (chỉ còn ở đơn chưa thanh toán & chưa đóng — ~99% dòng khi lướt lịch sử không còn nút); **cố ý KHÔNG thêm hộp thoại xác nhận** vì lúc mất phiếu thật thì cần in lại nhanh. ② cờ `isReprint` in `*** IN LẠI ***` ở **cả ba** bộ dựng phiếu (ESC/POS chữ · ESC/POS ảnh — bản đang dùng thật · HTML), và loa xướng `"In lại."` trước khi đọc món (`lib/speech.ts`). ③ tách khối cấp số thành `nextOrderNumber(tx, shiftId)` export từ `order.service.ts`; `POST /tables/sessions/:id/split` dùng nó thay `generateOrderNumber()` → hết mã lạc loài (trước đó **96 đơn** tách bàn mang mã cũ, không thuộc ca nào; tiền chốt ca **không hụt** vì `computeSummary` gom theo `payments.created_at`). ⚠️ Vì đơn tách nay thuộc ca nên nó **lọt vào `/orders/unprinted` và trạm quầy sẽ in** — mà tách bàn là chuyển món, nước đã pha rồi; nên ghi sẵn một dòng `order_prints` (`status='ok'`, `device_label='tach-ban-khong-can-in'`) ngay trong transaction tách.
- **Khoá đường xem ĐƠN CŨ sau mã code của chủ quán** (20/09/2026, `15a2616`+`9007cd2`): nhân viên tọc mạch đơn cũ — cả `cashier` lẫn `waiter` vào thẳng tab Đơn hàng, trang tự hỏi lại **mỗi 5 giây**, bày tên khách, tiền từng đơn, ai bấm đơn, từng lần thu tiền. **Vá:** mã băm argon2 ở `branches.settings.orders_gate.code_hash` (`maskBranchSecrets` che, giao diện chỉ thấy `code_set`; `mergeBranchSecrets` luôn giữ hash cũ kẻo quản lý bấm Lưu là mã bay) · đặt mã qua `PUT /api/settings/orders-gate` quyền **`org:update`** (cố ý KHÔNG dùng `PATCH /settings/branch` vì `settings:update` thì `branch_manager` cũng có → họ tự mở được khoá của chính mình) · `POST /orders-gate/verify` đổi mã lấy **vé JWT 5 phút** (`signOrdersGateToken`, claim `purpose="orders_gate"` — `verifyOrdersGateToken` BẮT BUỘC kiểm claim này, thiếu là access token thường cũng lọt vì cùng khoá ký) · middleware `requireOrdersGate` (`rbac.ts`) gắn lên **đúng 3 đường**: `GET /orders`, `GET /orders/:id`, `GET /tables/:id/history`. ⚠️ **KHÔNG cắt `orders:read`** — quyền đó gánh cả `/tables/takeaway`, `/orders/unprinted`, `/kitchen/orders`, `reprint`, `print-ack`, cắt là chết POS. ⚠️ **Chưa đặt mã = tính năng nằm im** (middleware đọc thêm 1 dòng `branches`; thiếu nhánh này thì vừa deploy là nhân viên gặp ô nhập mã mà gõ gì cũng qua). Web: vé **chỉ nằm trong RAM** (`stores/orders-gate-store.ts`, **cố ý không `persist`** — đây là chỗ thực hiện luật "mỗi lần vào phải nhập lại"), `apiFetch` gắn header `x-orders-gate` và tự vứt vé khi gặp 403 `ORDERS_GATE`; `components/orders-gate.tsx` bọc trang Đơn hàng và hộp thoại Lịch sử bàn. Chống dò mã đếm theo **từng người** (`rateLimiter` có thêm `keyFn`) 10 lần/5 phút — cả quán chung một IP nên đếm theo IP là khoá cả quán. Ghi vết bằng `logger` (`docker compose logs api | grep orders-gate`), chưa làm bảng nhật ký riêng. **Đã thử trọn vòng trên máy chủ thật, 9/9 đạt.**
